...
<h3 style="text-align: center">Electricity Bill Payment</h3>
    <div class="container py-5">
    
    <table class="table table-bordered table-hover" style="text-align: center">
    <tr style="background-color:azure">
    <th>State
    <th>Electricity Board
    <th>Consumer Number
    <th>Billing Unit
    <th>Miter Uint
    <th>Amount
...
try
{
	...
	pst=con.prepareStatement("select * from electricity where umobile=?");
	pst.setString(1, uid);
	rs=pst.executeQuery();
	...
	while(rs.next())
	{
		...
		<tr>
		<td><%=rs.getString("estate") %>
		<td><%=rs.getString("eprovider") %>
		<td><%=rs.getString("econsumer") %>
		<td><%=rs.getString("ebillingu") %>
		<td><%=rs.getInt("eunit") %>
		<td><%=rs.getString("eamount") %>
		<td><%=rs.getDate("edate") %>
		</tr>
